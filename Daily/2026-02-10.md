...
**결과**
- 세션 만료 → 자동 로그인 → SMS 수신 → MacroDroid가 자동 전달 → 인증 완료
- 58개 협력사 정상 조회 확인
...
- 선정산/주정산으로 잔액이 양수가 되어도 배치 실행 전까지 출금제한 상태가 유지되는 문제
...
**해결**
- `SettlementUploadUseCase.java` 수정
- 구체적인 메시지 반환: "20260128 ~ 20260203 정산서를 중복 업로드 하였습니다."

```java
private static final DateTimeFormatter PERIOD_FORMATTER = DateTimeFormatter.ofPattern("yyyyMMdd");
...
**문제**
- 프론트엔드에서 여전히 일반 메시지 "라이더 정산서 업로드에 실패했습니다" 표시
...
- 디버깅 시 실제 에러 객체 구조를 console.log로 확인하는 것이 중요
